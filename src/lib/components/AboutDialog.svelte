  ...
  <Dialog.Content class="w-full max-w-[400px] gap-0 overflow-hidden p-0">
    ...
    <div class="flex flex-col items-center gap-4 px-8 pt-8 pb-6 text-center">
      ...
      <div class="flex flex-col items-center gap-1">
        <h2 class="text-[15px] font-semibold tracking-tight text-foreground">Stroke</h2>
        <p class="text-[11px] text-muted-foreground/70">Database management, reimagined</p>
        <span class="mt-1 rounded-full border border-border/50 bg-muted/40 px-2.5 py-0.5 font-mono text-[10px] text-muted-foreground/60">
          v{appVersion}
        </span>
      </div>
    </div>
      ...
      {:else if isValid}
        ...
        <div class="flex flex-col gap-3">
          <div class="flex items-center gap-2.5">
            <div class="flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-500/12">
              <ShieldCheck class="size-3 text-emerald-500" />
            </div>
            <span class="text-[12px] font-medium text-foreground">Pro license active</span>
          </div>
          <div class="flex flex-col divide-y divide-border/40 rounded-lg border border-border/50 bg-muted/20 text-[11px]">
          ...
          <button
            ...
            class="w-full rounded-lg border border-border/50 py-1.5 text-[11px] text-muted-foreground/60 transition-colors hover:border-destructive/30 hover:text-destructive disabled:opacity-40"
          >
      ...
      {:else if isTrial}
        ...
        <div class="flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2.5">
              <div class="flex size-6 shrink-0 items-center justify-center rounded-full bg-amber-500/12">
                <Clock class="size-3 text-amber-500/80" />
              </div>
              <span class="text-[12px] font-medium text-foreground">Free trial</span>
            </div>
          ...
          {#if showActivate}
            ...
            <button
              ...
              class="text-[11px] text-muted-foreground/50 hover:text-muted-foreground transition-colors text-center"
            >Cancel</button>
          {:else}
            <button
              type="button"
              onclick={() => (showActivate = true)}
              class="flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-border/60 bg-muted/30 text-[11px] font-medium text-foreground/70 transition-colors hover:bg-muted/60 hover:text-foreground"
            >
      ...
      {:else}
        ...
        <div class="flex flex-col gap-3">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <p class="text-[12px] font-medium text-foreground">No active license</p>
              <p class="text-[10px] text-muted-foreground/60">Free features only</p>
            </div>
          </div>
          {#if showActivate}
            <LicenseActivation naked onactivated={() => { showActivate = false; open = false }} />
    ...
    <div class="flex items-center justify-between border-t border-border/40 px-6 py-3">
      <button
        ...
        class="text-[10px] text-muted-foreground/35 transition-colors hover:text-muted-foreground"
      >
        Report an issue
      </button>
      <div class="flex items-center gap-3.5">
        <a
          ...
          class="inline-flex items-center gap-1 text-[10px] text-muted-foreground/50 transition-colors hover:text-foreground"
        >
