...
{#if tokens.length > 0}
	...
	>
		{#each tokens as token}
			<kbd class="inline-flex h-[18px] min-w-[18px] items-center justify-center rounded-[5px] border border-border/60 bg-muted/40 px-1.5 font-mono text-ui-3xs font-medium leading-none text-muted-foreground shadow-[0_1px_0_color-mix(in_oklch,var(--border)_50%,transparent)] group-data-[selected]/command-item:border-border/70 group-data-[selected]/command-item:bg-muted/70 group-data-[selected]/command-item:text-foreground/90">
				{token}
			</kbd>
		{/each}
	</span>
