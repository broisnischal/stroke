      ...
      {:else}
        <div class="flex items-center gap-1">
          <input
            ...
            class="h-6 rounded-lg border-2 border-border bg-background/80 px-2 font-mono text-ui-xs outline-none placeholder:text-muted-foreground/40 focus:border-ring focus:ring-1 focus:ring-ring"
            style="width:150px"
            onkeydown={(e) => {
          ...
          <button
            type="button"
            class="inline-flex h-6 items-center gap-1 rounded-md bg-primary px-2 text-ui-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors"
          ...
          {#if grpData.charts.length > 0}
            <section>
              ...
              <div class="mb-3 flex items-center gap-2">
                ...
                {#if renamingGroupName === grpData.group}
                  <div class="flex items-center gap-1">
                    <input
                      ...
                      class="h-5 rounded-lg border-2 border-border bg-background/80 px-1.5 font-mono text-ui-xs outline-none focus:border-ring focus:ring-1 focus:ring-ring"
                      style="width:130px"
                      onkeydown={(e) => {
                ...
                {:else}
                  <h3 class="text-ui-sm font-semibold text-foreground/70">
                    {grpData.group}
                  </h3>
                  <span
                  ...
                  {#if grpData.group !== "Default"}
                    <button
                      type="button"
                      class={cn(btnIconSm, "size-5 ml-0.5")}
                      title="Rename group"
                    ...
                    <button
                      type="button"
                      class={cn(btnIconSm, "size-5 hover:text-destructive")}
                      title="Delete group"
                ...
                {#each grpData.charts as chart (chart.id)}
                  ...
                  >
                    ...
                    <div class="relative h-36 shrink-0 bg-muted/10">
                      ...
                      <div
                        class="absolute inset-0 flex items-end justify-between bg-gradient-to-t from-card/95 via-card/20 to-transparent p-2 opacity-0 transition-opacity group-hover:opacity-100"
                      >
                        <div class="flex items-center gap-1">
                          <button
                            type="button"
                        ...
                        <div class="flex items-center gap-0.5">
                          <button
                            ...
                            class={cn(
                              btnIconSm,
                              "size-6 bg-card/90 backdrop-blur-sm border border-border/40",
                            )}
                            title="Rename"
                          ...
                          <button
                            ...
                            title={confirmDeleteId === chart.id
                    ...
                    <div class="flex items-center gap-2 px-2.5 py-2">
                      {#if renamingChartId === chart.id}
                        <div class="flex min-w-0 flex-1 items-center gap-1">
                          <input
                            ...
                            class="h-5 min-w-0 flex-1 rounded-lg border-2 border-border bg-background/80 px-1.5 font-mono text-ui-xs outline-none focus:border-ring focus:ring-1 focus:ring-ring"
                            onkeydown={(e) => {
                      ...
                      {:else}
                        <span
                          class="min-w-0 flex-1 truncate text-ui-xs font-medium text-foreground/90"
                          title={chart.name}>{chart.name}</span
                        >
                      {/if}
                      <span
