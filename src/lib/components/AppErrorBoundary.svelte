...
<svelte:boundary onerror={(e) => { try { console.error('[app-boundary]', e) } catch { /* noop */ } }}>
  ...
  {#snippet failed(error, reset)}
    <div class="flex h-screen w-screen flex-col items-center justify-center gap-5 bg-background px-8 text-center select-none">
      ...
      <pre class="max-h-40 w-full max-w-lg overflow-auto rounded-lg border border-border/60 bg-muted/30 p-3 text-left font-mono text-[11px] leading-relaxed text-muted-foreground select-text">{String(/** @type {any} */ (error)?.message ?? error)}</pre>

      <div class="flex items-center gap-2">
        <button
          type="button"
          onclick={reset}
          class="inline-flex h-8 items-center gap-1.5 rounded-lg bg-foreground px-3 text-[12px] font-medium text-background transition-opacity hover:opacity-85"
        >
        ...
        <button
          ...
          class="inline-flex h-8 items-center rounded-lg border border-border/60 px-3 text-[12px] font-medium text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
        >
          Reload app
        </button>
      </div>
