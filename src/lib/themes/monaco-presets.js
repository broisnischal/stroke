/** @typedef {import('./registry.js').ThemeId} ThemeId */

/** @param {object} c */
function sqlRules(/** @type {{ comment: string, string: string, number: string, keyword: string, operator: string, identifier: string }} */ colors) {
  return [
    { token: 'comment', foreground: colors.comment, fontStyle: 'italic' },
    { token: 'string', foreground: colors.string },
    { token: 'string.sql', foreground: colors.string },
    { token: 'number', foreground: colors.number },
    { token: 'keyword', foreground: colors.keyword },
    { token: 'keyword.sql', foreground: colors.keyword },
    { token: 'operator', foreground: colors.operator },
    { token: 'identifier', foreground: colors.identifier },
  ]
}

/** @param {object} c */
function darkChrome(c) {
  return {
    'editor.background': c.background,
    'editor.foreground': c.foreground,
    'editorLineNumber.foreground': c.lineNumber,
    'editorLineNumber.activeForeground': c.lineNumberActive,
    'editor.selectionBackground': c.selection,
    'editor.inactiveSelectionBackground': c.selectionInactive,
    'editor.lineHighlightBackground': c.lineHighlight,
    'editor.lineHighlightBorder': '#00000000',
    'editorCursor.foreground': c.cursor,
    'editorWidget.background': c.widgetBg,
    'editorWidget.border': c.widgetBorder,
    'editorIndentGuide.background': c.indent,
    'editorIndentGuide.activeBackground': c.indentActive,
    'editorBracketMatch.background': c.bracketBg,
    'editorBracketMatch.border': c.bracketBorder,
  }
}

/** @type {Record<ThemeId, { base: 'vs' | 'vs-dark', chrome: object, rules: object[] }>} */
export const MONACO_THEME_SPECS = {
  'moss-light': {
    base: 'vs',
    chrome: {
      background: '#f9fbf7',
      foreground: '#151c13',
      lineNumber: '#6c746a',
      lineNumberActive: '#31382f',
      lineHighlight: '#eaeee7',
      selection: '#116d452e',
      selectionInactive: '#116d4518',
      cursor: '#116d45',
      widgetBg: '#fdfefc',
      widgetBorder: '#cad0c4',
      indent: '#dae0d4',
      indentActive: '#c6ccbf',
      bracketBg: '#116d4526',
      bracketBorder: '#50584e',
    },
    rules: sqlRules({
      comment: '565D54',
      string: '0A6479',
      number: '784C00',
      keyword: '116D45',
      operator: '363F35',
      identifier: '151C13',
    }),
  },
  'moss': {
    base: 'vs-dark',
    chrome: {
      background: '#0c0f0b',
      foreground: '#e8ede6',
      lineNumber: '#888e86',
      lineNumberActive: '#c9d0c7',
      lineHighlight: '#161a15',
      selection: '#78c59433',
      selectionInactive: '#78c5941c',
      cursor: '#78c594',
      widgetBg: '#121611',
      widgetBorder: '#2a2f28',
      indent: '#20251f',
      indentActive: '#2e332c',
      bracketBg: '#78c5942b',
      bracketBorder: '#a3aaa1',
    },
    rules: sqlRules({
      comment: '9DA49B',
      string: '77C5DA',
      number: 'E4AF6C',
      keyword: '78C594',
      operator: 'CED2CC',
      identifier: 'E8EDE6',
    }),
  },
  'orchid-light': {
    base: 'vs',
    chrome: {
      background: '#fcf9fc',
      foreground: '#221620',
      lineNumber: '#7b6d78',
      lineNumberActive: '#3e313b',
      lineHighlight: '#f2ebf1',
      selection: '#9535852e',
      selectionInactive: '#95358518',
      cursor: '#953585',
      widgetBg: '#fffdff',
      widgetBorder: '#d7cbd5',
      indent: '#e7dbe6',
      indentActive: '#d3c6d2',
      bracketBg: '#95358526',
      bracketBorder: '#5e515c',
    },
    rules: sqlRules({
      comment: '645761',
      string: '984026',
      number: '7F5100',
      keyword: '953585',
      operator: '453844',
      identifier: '221620',
    }),
  },
  'orchid': {
    base: 'vs-dark',
    chrome: {
      background: '#100c11',
      foreground: '#efe9f0',
      lineNumber: '#928992',
      lineNumberActive: '#d4cad4',
      lineHighlight: '#1c171c',
      selection: '#e492c933',
      selectionInactive: '#e492c91c',
      cursor: '#e492c9',
      widgetBg: '#181218',
      widgetBorder: '#322a32',
      indent: '#282128',
      indentActive: '#362e36',
      bracketBg: '#e492c92b',
      bracketBorder: '#aea4ae',
    },
    rules: sqlRules({
      comment: 'A89EA8',
      string: 'EBA488',
      number: 'E4AF6C',
      keyword: 'E492C9',
      operator: 'D5CED5',
      identifier: 'EFE9F0',
    }),
  },
  'matte': {
    base: 'vs-dark',
    chrome: {
      background: '#141414',
      foreground: '#d9d9d9',
      lineNumber: '#696969',
      lineNumberActive: '#bebebe',
      lineHighlight: '#212121',
      selection: '#ffffff1f',
      selectionInactive: '#ffffff12',
      cursor: '#dcdcdc',
      widgetBg: '#1f1f1f',
      widgetBorder: '#343434',
      indent: '#2a2a2a',
      indentActive: '#3a3a3a',
      bracketBg: '#ffffff1a',
      bracketBorder: '#747474',
    },
    rules: sqlRules({
      comment: '6C6C6C',
      string: 'C4C4C4',
      number: '9B9B9B',
      keyword: 'F7F7F7',
      operator: '8C8C8C',
      identifier: 'D9D9D9',
    }),
  },
  'claude-light': {
    base: 'vs',
    chrome: {
      background: '#f6f4f0',
      foreground: '#1b1915',
      lineNumber: '#75716a',
      lineNumberActive: '#38352f',
      lineHighlight: '#e9e7e0',
      selection: '#af56292e',
      selectionInactive: '#af562918',
      cursor: '#af5629',
      widgetBg: '#f9f8f4',
      widgetBorder: '#ccc8bd',
      indent: '#dcd8cd',
      indentActive: '#c8c4b8',
      bracketBg: '#af562926',
      bracketBorder: '#59554e',
    },
    rules: sqlRules({
      comment: '5E5A53',
      string: '777423',
      number: '926218',
      keyword: 'AF5629',
      operator: '3E3B37',
      identifier: '1B1915',
    }),
  },
  'claude': {
    base: 'vs-dark',
    chrome: {
      background: '#151412',
      foreground: '#f1f0ec',
      lineNumber: '#908d88',
      lineNumberActive: '#d2cfca',
      lineHighlight: '#201f1c',
      selection: '#e18b6a33',
      selectionInactive: '#e18b6a1c',
      cursor: '#e18b6a',
      widgetBg: '#1c1b19',
      widgetBorder: '#34332f',
      indent: '#2a2926',
      indentActive: '#393733',
      bracketBg: '#e18b6a2b',
      bracketBorder: '#aba9a4',
    },
    rules: sqlRules({
      comment: 'A5A39D',
      string: 'B4AE69',
      number: 'D7A360',
      keyword: 'E18B6A',
      operator: 'D7D6D2',
      identifier: 'F1F0EC',
    }),
  },
  'dark-high-contrast': {
    base: 'vs-dark',
    chrome: {
      background: '#000000', foreground: '#ffffff', lineNumber: '#b0b0b0', lineNumberActive: '#ffffff',
      lineHighlight: '#1c1c1c', selection: '#3a5a99cc', selectionInactive: '#3a5a9966', cursor: '#ffe000',
      widgetBg: '#0a0a0a', widgetBorder: '#8a8a8a', indent: '#3a3a3a', indentActive: '#8a8a8a',
      bracketBg: '#333333', bracketBorder: '#b0b0b0',
    },
    rules: [
      { token: 'comment', foreground: 'b0b0b0', fontStyle: 'italic' },
      { token: 'string', foreground: '5cff5c' }, { token: 'string.sql', foreground: '5cff5c' },
      { token: 'number', foreground: '5cd6ff' },
      { token: 'keyword', foreground: 'ffe000' }, { token: 'keyword.sql', foreground: 'ffe000' },
      { token: 'operator', foreground: 'ffffff' }, { token: 'identifier', foreground: 'ffffff' },
    ],
  },
  'light-high-contrast': {
    base: 'vs',
    chrome: {
      background: '#ffffff', foreground: '#000000', lineNumber: '#4a4a4a', lineNumberActive: '#000000',
      lineHighlight: '#eaeaea', selection: '#9ec5ffcc', selectionInactive: '#9ec5ff66', cursor: '#1a3fd0',
      widgetBg: '#ffffff', widgetBorder: '#5a5a5a', indent: '#c0c0c0', indentActive: '#5a5a5a',
      bracketBg: '#dddddd', bracketBorder: '#4a4a4a',
    },
    rules: [
      { token: 'comment', foreground: '4a4a4a', fontStyle: 'italic' },
      { token: 'string', foreground: '006000' }, { token: 'string.sql', foreground: '006000' },
      { token: 'number', foreground: '00408a' },
      { token: 'keyword', foreground: '1a3fd0' }, { token: 'keyword.sql', foreground: '1a3fd0' },
      { token: 'operator', foreground: '000000' }, { token: 'identifier', foreground: '000000' },
    ],
  },
  light: {
    base: 'vs',
    chrome: {
      background: '#fafafa',
      foreground: '#1a1a1a',
      lineNumber: '#a0a0a0',
      lineNumberActive: '#555555',
      lineHighlight: '#f0f0f0',
      selection: '#1a1a1a20',
      selectionInactive: '#1a1a1a10',
      cursor: '#1a1a1a',
      widgetBg: '#f5f5f5',
      widgetBorder: '#d8d8d8',
      indent: '#e0e0e0',
      indentActive: '#c8c8c8',
      bracketBg: '#e0e0e088',
      bracketBorder: '#a0a0a0',
    },
    rules: [
      { token: 'comment', foreground: '8a8a8a', fontStyle: 'italic' },
      { token: 'string', foreground: '1a6b3a' },
      { token: 'string.sql', foreground: '1a6b3a' },
      { token: 'number', foreground: '1060b0' },
      { token: 'keyword', foreground: '8a3a00' },
      { token: 'keyword.sql', foreground: '8a3a00' },
      { token: 'operator', foreground: '404040' },
      { token: 'identifier', foreground: '1a1a1a' },
    ],
  },
  parchment: {
    base: 'vs',
    chrome: {
      background: '#faf7f0',
      foreground: '#2a1c10',
      lineNumber: '#b0a48c',
      lineNumberActive: '#6a5038',
      lineHighlight: '#ede9de',
      selection: '#3a50c420',
      selectionInactive: '#3a50c410',
      cursor: '#2a1c10',
      widgetBg: '#f5f0e6',
      widgetBorder: '#d6cec0',
      indent: '#e0d8c8',
      indentActive: '#c8bcaa',
      bracketBg: '#e0d8c888',
      bracketBorder: '#b0a48c',
    },
    rules: [
      { token: 'comment', foreground: 'a09070', fontStyle: 'italic' },
      { token: 'string', foreground: '1e6a48' },
      { token: 'string.sql', foreground: '1e6a48' },
      { token: 'number', foreground: '1850a0' },
      { token: 'keyword', foreground: '3a50c4' },
      { token: 'keyword.sql', foreground: '3a50c4' },
      { token: 'operator', foreground: '5a4030' },
      { token: 'identifier', foreground: '2a1c10' },
    ],
  },
  ice: {
    base: 'vs',
    chrome: {
      background: '#f2f5fc',
      foreground: '#0e1a30',
      lineNumber: '#8090b8',
      lineNumberActive: '#3a5090',
      lineHighlight: '#e4eaf6',
      selection: '#2f5fcc22',
      selectionInactive: '#2f5fcc12',
      cursor: '#0e1a30',
      widgetBg: '#eaeff8',
      widgetBorder: '#c0cce4',
      indent: '#d4dcf0',
      indentActive: '#b8c8e8',
      bracketBg: '#d4dcf088',
      bracketBorder: '#8090b8',
    },
    rules: [
      { token: 'comment', foreground: '7090b8', fontStyle: 'italic' },
      { token: 'string', foreground: '1a7a50' },
      { token: 'string.sql', foreground: '1a7a50' },
      { token: 'number', foreground: '1050c0' },
      { token: 'keyword', foreground: '2f5fcc' },
      { token: 'keyword.sql', foreground: '2f5fcc' },
      { token: 'operator', foreground: '304060' },
      { token: 'identifier', foreground: '0e1a30' },
    ],
  },
  dark: {
    base: 'vs-dark',
    chrome: {
      background: '#1c1c1c',
      foreground: '#ebebeb',
      lineNumber: '#6b7280',
      lineNumberActive: '#9ca3af',
      lineHighlight: '#262626',
      selection: '#3b4a5c99',
      selectionInactive: '#3b4a5c44',
      cursor: '#f5f5f5',
      widgetBg: '#232323',
      widgetBorder: '#363636',
      indent: '#2e2e2e',
      indentActive: '#454545',
      bracketBg: '#40404088',
      bracketBorder: '#6b7280',
    },
    rules: [
      { token: 'comment', foreground: '6b7280', fontStyle: 'italic' },
      { token: 'string', foreground: '7ee787' },
      { token: 'string.sql', foreground: '7ee787' },
      { token: 'number', foreground: '79c0ff' },
      { token: 'keyword', foreground: 'c4b5fd' },
      { token: 'keyword.sql', foreground: 'c4b5fd' },
      { token: 'operator', foreground: 'd1d5db' },
      { token: 'identifier', foreground: 'ebebeb' },
    ],
  },
  graphite: {
    base: 'vs-dark',
    chrome: {
      background: '#1e1f22',
      foreground: '#dfe1e5',
      lineNumber: '#5c5f66',
      lineNumberActive: '#9da1a8',
      lineHighlight: '#26282d',
      selection: '#3574f04d',
      selectionInactive: '#3574f026',
      cursor: '#dfe1e5',
      widgetBg: '#26282c',
      widgetBorder: '#37393e',
      indent: '#2c2e33',
      indentActive: '#43464c',
      bracketBg: '#3a3d4288',
      bracketBorder: '#5c5f66',
    },
    rules: [
      { token: 'comment', foreground: '6f737a', fontStyle: 'italic' },
      { token: 'string', foreground: '7ee787' },
      { token: 'string.sql', foreground: '7ee787' },
      { token: 'number', foreground: '79c0ff' },
      { token: 'keyword', foreground: 'c4b5fd' },
      { token: 'keyword.sql', foreground: 'c4b5fd' },
      { token: 'operator', foreground: 'd1d5db' },
      { token: 'identifier', foreground: 'dfe1e5' },
    ],
  },
  vitesse: {
    base: 'vs-dark',
    chrome: {
      background: '#121212',
      foreground: '#dbd7ca',
      lineNumber: '#758575',
      lineNumberActive: '#b8a965',
      lineHighlight: '#1a1916',
      selection: '#4d937544',
      selectionInactive: '#4d937522',
      cursor: '#dbd7ca',
      widgetBg: '#1a1916',
      widgetBorder: '#363430',
      indent: '#2a2824',
      indentActive: '#4a4844',
      bracketBg: '#4d937533',
      bracketBorder: '#758575',
    },
    rules: [
      { token: 'comment', foreground: '758575', fontStyle: 'italic' },
      { token: 'string', foreground: 'c98a7d' },
      { token: 'string.sql', foreground: 'c98a7d' },
      { token: 'number', foreground: '4C9A91' },
      { token: 'keyword', foreground: '4d9375' },
      { token: 'keyword.sql', foreground: '4d9375' },
      { token: 'operator', foreground: 'cb7676' },
      { token: 'identifier', foreground: 'dbd7ca' },
    ],
  },
  midnight: {
    base: 'vs-dark',
    chrome: {
      background: '#0f1419',
      foreground: '#e6edf3',
      lineNumber: '#6e7681',
      lineNumberActive: '#8b949e',
      lineHighlight: '#161b22',
      selection: '#388bfd44',
      selectionInactive: '#388bfd22',
      cursor: '#e6edf3',
      widgetBg: '#161b22',
      widgetBorder: '#30363d',
      indent: '#21262d',
      indentActive: '#484f58',
      bracketBg: '#388bfd33',
      bracketBorder: '#6e7681',
    },
    rules: [
      { token: 'comment', foreground: '6e7681', fontStyle: 'italic' },
      { token: 'string', foreground: 'a5d6ff' },
      { token: 'string.sql', foreground: 'a5d6ff' },
      { token: 'number', foreground: '79c0ff' },
      { token: 'keyword', foreground: 'ff7b72' },
      { token: 'keyword.sql', foreground: 'ff7b72' },
      { token: 'operator', foreground: 'd2a8ff' },
      { token: 'identifier', foreground: 'e6edf3' },
    ],
  },
  slate: {
    base: 'vs-dark',
    chrome: {
      background: '#1a1a1f',
      foreground: '#e4e4e7',
      lineNumber: '#71717a',
      lineNumberActive: '#a1a1aa',
      lineHighlight: '#232329',
      selection: '#a78bfa44',
      selectionInactive: '#a78bfa22',
      cursor: '#fafafa',
      widgetBg: '#232329',
      widgetBorder: '#3f3f46',
      indent: '#2a2a32',
      indentActive: '#52525b',
      bracketBg: '#a78bfa33',
      bracketBorder: '#71717a',
    },
    rules: [
      { token: 'comment', foreground: '71717a', fontStyle: 'italic' },
      { token: 'string', foreground: '86efac' },
      { token: 'string.sql', foreground: '86efac' },
      { token: 'number', foreground: '93c5fd' },
      { token: 'keyword', foreground: 'c4b5fd' },
      { token: 'keyword.sql', foreground: 'c4b5fd' },
      { token: 'operator', foreground: 'd4d4d8' },
      { token: 'identifier', foreground: 'e4e4e7' },
    ],
  },
  nord: {
    base: 'vs-dark',
    chrome: {
      background: '#2e3440',
      foreground: '#eceff4',
      lineNumber: '#616e88',
      lineNumberActive: '#d8dee9',
      lineHighlight: '#3b4252',
      selection: '#88c0d044',
      selectionInactive: '#88c0d022',
      cursor: '#eceff4',
      widgetBg: '#3b4252',
      widgetBorder: '#4c566a',
      indent: '#434c5e',
      indentActive: '#616e88',
      bracketBg: '#88c0d033',
      bracketBorder: '#616e88',
    },
    rules: [
      { token: 'comment', foreground: '616E88', fontStyle: 'italic' },
      { token: 'string', foreground: 'A3BE8C' },
      { token: 'string.sql', foreground: 'A3BE8C' },
      { token: 'number', foreground: 'B48EAD' },
      { token: 'keyword', foreground: '81A1C1' },
      { token: 'keyword.sql', foreground: '81A1C1' },
      { token: 'operator', foreground: 'EBCB8B' },
      { token: 'identifier', foreground: 'ECEFF4' },
    ],
  },
  dracula: {
    base: 'vs-dark',
    chrome: {
      background: '#282a36',
      foreground: '#f8f8f2',
      lineNumber: '#6272a4',
      lineNumberActive: '#f8f8f2',
      lineHighlight: '#313341',
      selection: '#bd93f944',
      selectionInactive: '#bd93f922',
      cursor: '#f8f8f2',
      widgetBg: '#313341',
      widgetBorder: '#44475a',
      indent: '#383a4a',
      indentActive: '#6272a4',
      bracketBg: '#bd93f933',
      bracketBorder: '#6272a4',
    },
    rules: [
      { token: 'comment', foreground: '6272A4', fontStyle: 'italic' },
      { token: 'string', foreground: 'F1FA8C' },
      { token: 'string.sql', foreground: 'F1FA8C' },
      { token: 'number', foreground: 'BD93F9' },
      { token: 'keyword', foreground: 'FF79C6' },
      { token: 'keyword.sql', foreground: 'FF79C6' },
      { token: 'operator', foreground: 'FF79C6' },
      { token: 'identifier', foreground: 'F8F8F2' },
    ],
  },
  mono: {
    base: 'vs-dark',
    chrome: {
      background: '#000000',
      foreground: '#fafafa',
      lineNumber: '#555555',
      lineNumberActive: '#888888',
      lineHighlight: '#0f0f0f',
      selection: '#ffffff20',
      selectionInactive: '#ffffff10',
      cursor: '#fafafa',
      widgetBg: '#0d0d0d',
      widgetBorder: '#1e1e1e',
      indent: '#181818',
      indentActive: '#2a2a2a',
      bracketBg: '#ffffff18',
      bracketBorder: '#555555',
    },
    rules: sqlRules({
      comment: '555555',
      string: '7ee787',
      number: '79c0ff',
      keyword: 'c4b5fd',
      operator: 'd4d4d4',
      identifier: 'fafafa',
    }),
  },
  forest: {
    base: 'vs-dark',
    chrome: {
      background: '#141a14',
      foreground: '#e2ebe0',
      lineNumber: '#6b7d68',
      lineNumberActive: '#9cb396',
      lineHighlight: '#1c241c',
      selection: '#7dd3a044',
      selectionInactive: '#7dd3a022',
      cursor: '#e2ebe0',
      widgetBg: '#1c241c',
      widgetBorder: '#2a362a',
      indent: '#243024',
      indentActive: '#3d4f3d',
      bracketBg: '#7dd3a033',
      bracketBorder: '#6b7d68',
    },
    rules: [
      { token: 'comment', foreground: '6B7D68', fontStyle: 'italic' },
      { token: 'string', foreground: '9AE6B4' },
      { token: 'string.sql', foreground: '9AE6B4' },
      { token: 'number', foreground: '7DD3A0' },
      { token: 'keyword', foreground: '68D391' },
      { token: 'keyword.sql', foreground: '68D391' },
      { token: 'operator', foreground: 'C6F6D5' },
      { token: 'identifier', foreground: 'E2EBE0' },
    ],
  },
  solarized: {
    base: 'vs-dark',
    chrome: {
      background: '#002b36',
      foreground: '#93a1a1',
      lineNumber: '#586e75',
      lineNumberActive: '#839496',
      lineHighlight: '#073642',
      selection: '#2aa19844',
      selectionInactive: '#2aa19822',
      cursor: '#839496',
      widgetBg: '#073642',
      widgetBorder: '#094552',
      indent: '#073642',
      indentActive: '#586e75',
      bracketBg: '#2aa19833',
      bracketBorder: '#586e75',
    },
    rules: sqlRules({
      comment: '586E75',
      string: '2AA198',
      number: 'D33682',
      keyword: '859900',
      operator: 'CB4B16',
      identifier: 'EEE8D5',
    }),
  },
  linear: {
    base: 'vs-dark',
    chrome: {
      background: '#0d0d0f',
      foreground: '#ededed',
      lineNumber: '#6b6b70',
      lineNumberActive: '#a0a0a8',
      lineHighlight: '#161618',
      selection: '#5e6ad244',
      selectionInactive: '#5e6ad222',
      cursor: '#ededed',
      widgetBg: '#141415',
      widgetBorder: '#232326',
      indent: '#1a1a1d',
      indentActive: '#2a2a2e',
      bracketBg: '#5e6ad233',
      bracketBorder: '#6b6b70',
    },
    rules: sqlRules({
      comment: '6B6B70',
      string: 'A5B4FC',
      number: '8B9DFF',
      keyword: 'C4B5FD',
      operator: 'D4D4D8',
      identifier: 'EDEDED',
    }),
  },
  cursor: {
    base: 'vs-dark',
    chrome: {
      background: '#141414',
      foreground: '#e4e4e4',
      lineNumber: '#6e6e6e',
      lineNumberActive: '#a0a0a0',
      lineHighlight: '#1c1c1c',
      selection: '#528bff44',
      selectionInactive: '#528bff22',
      cursor: '#e4e4e4',
      widgetBg: '#1c1c1c',
      widgetBorder: '#262626',
      indent: '#1f1f1f',
      indentActive: '#333333',
      bracketBg: '#528bff33',
      bracketBorder: '#6e6e6e',
    },
    rules: sqlRules({
      comment: '6E6E6E',
      string: '7ee787',
      number: '79c0ff',
      keyword: 'c4b5fd',
      operator: 'd4d4d4',
      identifier: 'e4e4e4',
    }),
  },
  vscode: {
    base: 'vs-dark',
    chrome: {
      background: '#1e1e1e',
      foreground: '#cccccc',
      lineNumber: '#858585',
      lineNumberActive: '#c6c6c6',
      lineHighlight: '#2a2d2e',
      selection: '#264f7844',
      selectionInactive: '#264f7822',
      cursor: '#aeafad',
      widgetBg: '#252526',
      widgetBorder: '#454545',
      indent: '#404040',
      indentActive: '#707070',
      bracketBg: '#007acc33',
      bracketBorder: '#858585',
    },
    rules: sqlRules({
      comment: '6A9955',
      string: 'ce9178',
      number: 'b5cea8',
      keyword: '569cd6',
      operator: 'd4d4d4',
      identifier: 'cccccc',
    }),
  },
  github: {
    base: 'vs-dark',
    chrome: {
      background: '#0d1117',
      foreground: '#e6edf3',
      lineNumber: '#6e7681',
      lineNumberActive: '#8b949e',
      lineHighlight: '#161b22',
      selection: '#388bfd44',
      selectionInactive: '#388bfd22',
      cursor: '#e6edf3',
      widgetBg: '#161b22',
      widgetBorder: '#30363d',
      indent: '#21262d',
      indentActive: '#484f58',
      bracketBg: '#388bfd33',
      bracketBorder: '#6e7681',
    },
    rules: sqlRules({
      comment: '8B949E',
      string: 'A5D6FF',
      number: '79C0FF',
      keyword: 'FF7B72',
      operator: 'D2A8FF',
      identifier: 'E6EDF3',
    }),
  },
  onedark: {
    base: 'vs-dark',
    chrome: {
      background: '#282c34',
      foreground: '#abb2bf',
      lineNumber: '#5c6370',
      lineNumberActive: '#abb2bf',
      lineHighlight: '#2c313a',
      selection: '#528bff44',
      selectionInactive: '#528bff22',
      cursor: '#528bff',
      widgetBg: '#2c313a',
      widgetBorder: '#353b45',
      indent: '#3b4048',
      indentActive: '#5c6370',
      bracketBg: '#528bff33',
      bracketBorder: '#5c6370',
    },
    rules: sqlRules({
      comment: '5C6370',
      string: '98C379',
      number: 'D19A66',
      keyword: 'C678DD',
      operator: '56B6C2',
      identifier: 'ABB2BF',
    }),
  },
  catppuccin: {
    base: 'vs-dark',
    chrome: {
      background: '#1e1e2e',
      foreground: '#cdd6f4',
      lineNumber: '#6c7086',
      lineNumberActive: '#a6adc8',
      lineHighlight: '#313244',
      selection: '#cba6f744',
      selectionInactive: '#cba6f722',
      cursor: '#f5e0dc',
      widgetBg: '#313244',
      widgetBorder: '#45475a',
      indent: '#313244',
      indentActive: '#585b70',
      bracketBg: '#cba6f733',
      bracketBorder: '#6c7086',
    },
    rules: sqlRules({
      comment: '6C7086',
      string: 'A6E3A1',
      number: 'FAB387',
      keyword: 'CBA6F7',
      operator: '89DCEB',
      identifier: 'CDD6F4',
    }),
  },
  // Comment colours are the ones worth naming: at 3.9:1 (dark) and 3.6:1
  // (light) the first drafts were the only tokens under the 4.5:1 floor, which
  // is backwards - a comment is prose and gets read more carefully than a
  // keyword you pattern-match on. Both lifted until every token passes.
  hotdog: {
    base: 'vs-dark',
    chrome: {
      background: '#2b0707',
      foreground: '#ffd83d',
      lineNumber: '#a8632c',
      lineNumberActive: '#ffb02e',
      lineHighlight: '#3d0c0c',
      selection: '#ffd83d33',
      selectionInactive: '#ffd83d1c',
      cursor: '#ffd83d',
      widgetBg: '#3d0c0c',
      widgetBorder: '#6b1c1c',
      indent: '#4a1010',
      indentActive: '#7a2020',
      bracketBg: '#ffd83d2b',
      bracketBorder: '#a8632c',
    },
    rules: sqlRules({
      comment: 'C98A4B',
      string: 'FFB02E',
      number: 'FF8A6B',
      keyword: 'FFD83D',
      operator: 'E8A83C',
      identifier: 'FFE98A',
    }),
  },
  obsidian: {
    base: 'vs-dark',
    chrome: {
      background: '#14161b',
      foreground: '#eeeff4',
      lineNumber: '#5b6270',
      lineNumberActive: '#9ba2b0',
      lineHighlight: '#1b1e25',
      selection: '#6fd3dc33',
      selectionInactive: '#6fd3dc1c',
      cursor: '#6fd3dc',
      widgetBg: '#1b1e25',
      widgetBorder: '#2f343e',
      indent: '#22262e',
      indentActive: '#3c424e',
      bracketBg: '#6fd3dc2b',
      bracketBorder: '#5b6270',
    },
    rules: sqlRules({
      comment: '848B99',
      string: '7FD4C1',
      number: 'E8C07A',
      keyword: '6FD3DC',
      operator: 'B8BDC7',
      identifier: 'EEEFF4',
    }),
  },
  clay: {
    base: 'vs',
    chrome: {
      background: '#faf6ef',
      foreground: '#2b2119',
      lineNumber: '#a9a094',
      lineNumberActive: '#6b6055',
      lineHighlight: '#f2ece1',
      selection: '#a856312e',
      selectionInactive: '#a8563118',
      cursor: '#a85631',
      widgetBg: '#fffdf9',
      widgetBorder: '#ddd4c6',
      indent: '#ece4d7',
      indentActive: '#cfc4b3',
      bracketBg: '#a8563126',
      bracketBorder: '#a9a094',
    },
    rules: sqlRules({
      comment: '756B5E',
      string: '4F6B3F',
      number: '9A6216',
      keyword: 'A85631',
      operator: '6B6055',
      identifier: '2B2119',
    }),
  },
  'rose-pine': {
    base: 'vs-dark',
    chrome: {
      background: '#191724',
      foreground: '#e0def4',
      lineNumber: '#6e6a86',
      lineNumberActive: '#908caa',
      lineHighlight: '#1f1d2e',
      selection: '#c4a7e744',
      selectionInactive: '#c4a7e722',
      cursor: '#e0def4',
      widgetBg: '#1f1d2e',
      widgetBorder: '#403d52',
      indent: '#26233a',
      indentActive: '#524f67',
      bracketBg: '#c4a7e733',
      bracketBorder: '#6e6a86',
    },
    rules: sqlRules({
      comment: '6E6A86',
      string: '9CCFD8',
      number: 'F6C177',
      keyword: 'C4A7E7',
      operator: 'EBBCBA',
      identifier: 'E0DEF4',
    }),
  },
  raycast: {
    base: 'vs-dark',
    chrome: {
      background: '#0a0a0a',
      foreground: '#ededed',
      lineNumber: '#666666',
      lineNumberActive: '#999999',
      lineHighlight: '#141414',
      selection: '#ff636344',
      selectionInactive: '#ff636322',
      cursor: '#ededed',
      widgetBg: '#141414',
      widgetBorder: '#222222',
      indent: '#1a1a1a',
      indentActive: '#333333',
      bracketBg: '#ff636333',
      bracketBorder: '#666666',
    },
    rules: sqlRules({
      comment: '666666',
      string: 'ffb4b4',
      number: 'ff8a8a',
      keyword: 'ff6363',
      operator: 'd4d4d4',
      identifier: 'ededed',
    }),
  },
  vercel: {
    base: 'vs-dark',
    chrome: {
      background: '#000000',
      foreground: '#ededed',
      lineNumber: '#666666',
      lineNumberActive: '#999999',
      lineHighlight: '#111111',
      selection: '#0070f344',
      selectionInactive: '#0070f322',
      cursor: '#ededed',
      widgetBg: '#111111',
      widgetBorder: '#1a1a1a',
      indent: '#111111',
      indentActive: '#333333',
      bracketBg: '#0070f333',
      bracketBorder: '#666666',
    },
    rules: sqlRules({
      comment: '666666',
      string: '76baff',
      number: '3291ff',
      keyword: '0070f3',
      operator: 'd4d4d4',
      identifier: 'ededed',
    }),
  },
  notion: {
    base: 'vs',
    chrome: {
      background: '#ffffff',
      foreground: '#37352f',
      lineNumber: '#b4b4b0',
      lineNumberActive: '#787774',
      lineHighlight: '#f7f6f3',
      selection: '#2383e220',
      selectionInactive: '#2383e210',
      cursor: '#37352f',
      widgetBg: '#ffffff',
      widgetBorder: '#e9e9e7',
      indent: '#efefec',
      indentActive: '#d4d4d0',
      bracketBg: '#2383e218',
      bracketBorder: '#b4b4b0',
    },
    rules: sqlRules({
      comment: '9B9A97',
      string: '0B6E99',
      number: 'D9730D',
      keyword: 'D44C47',
      operator: '37352F',
      identifier: '37352F',
    }),
  },
  'github-light': {
    base: 'vs',
    chrome: {
      background: '#ffffff',
      foreground: '#1f2328',
      lineNumber: '#8c959f',
      lineNumberActive: '#656d76',
      lineHighlight: '#f6f8fa',
      selection: '#0969da22',
      selectionInactive: '#0969da11',
      cursor: '#1f2328',
      widgetBg: '#ffffff',
      widgetBorder: '#d0d7de',
      indent: '#eaeef2',
      indentActive: '#d0d7de',
      bracketBg: '#0969da18',
      bracketBorder: '#8c959f',
    },
    rules: sqlRules({
      comment: '6E7781',
      string: '0A3069',
      number: '0550AE',
      keyword: 'CF222E',
      operator: '953800',
      identifier: '1F2328',
    }),
  },
  'vscode-light': {
    base: 'vs',
    chrome: {
      background: '#ffffff',
      foreground: '#333333',
      lineNumber: '#afafaf',
      lineNumberActive: '#6e6e6e',
      lineHighlight: '#f5f5f5',
      selection: '#add6ff',
      selectionInactive: '#e5ebf1',
      cursor: '#000000',
      widgetBg: '#f3f3f3',
      widgetBorder: '#e5e5e5',
      indent: '#eeeeee',
      indentActive: '#d4d4d4',
      bracketBg: '#007acc22',
      bracketBorder: '#afafaf',
    },
    rules: sqlRules({
      comment: '008000',
      string: 'a31515',
      number: '098658',
      keyword: '0000ff',
      operator: '333333',
      identifier: '333333',
    }),
  },
  'linear-light': {
    base: 'vs',
    chrome: {
      background: '#ffffff',
      foreground: '#3f3f46',
      lineNumber: '#a1a1aa',
      lineNumberActive: '#71717a',
      lineHighlight: '#f4f4f5',
      selection: '#5e6ad220',
      selectionInactive: '#5e6ad210',
      cursor: '#3f3f46',
      widgetBg: '#ffffff',
      widgetBorder: '#ececee',
      indent: '#f4f4f5',
      indentActive: '#e4e4e7',
      bracketBg: '#5e6ad218',
      bracketBorder: '#a1a1aa',
    },
    rules: sqlRules({
      comment: 'A1A1AA',
      string: '4F5ABF',
      number: '5E6AD2',
      keyword: '7C3AED',
      operator: '52525B',
      identifier: '3F3F46',
    }),
  },
}

/** @param {ThemeId} id */
/**
 * JSON token colours, derived from whatever the theme already says.
 *
 * Every preset here was written for SQL - `keyword`, `string`, `number`,
 * `identifier`. Monaco's JSON tokenizer emits its own scopes (`string.key.json`
 * for a property name, `string.value.json` for a string value, `keyword.json`
 * for true/false/null), and a theme that names none of them leaves the JSON
 * viewer to fall back to the editor foreground for nearly everything - a wall
 * of one colour, correctly indented and unreadable.
 *
 * Keys deliberately take the keyword colour rather than the string colour:
 * in a result set the property names are the structure and the values are the
 * data, and colouring both the same is what made them impossible to scan.
 *
 * @param {object[]} rules the theme's own rules, read for its palette
 */
function jsonRules(rules) {
  const of = (/** @type {string} */ token) =>
    /** @type {any} */ (rules.find((r) => /** @type {any} */ (r).token === token))?.foreground
  const string = of('string')
  const number = of('number')
  const keyword = of('keyword')
  const operator = of('operator')
  const out = []
  const add = (/** @type {string} */ token, /** @type {string|undefined} */ fg) => {
    if (fg) out.push({ token, foreground: fg })
  }
  add('string.key.json', keyword)
  add('string.value.json', string)
  add('number.json', number)
  add('keyword.json', keyword)
  add('delimiter.bracket.json', operator)
  add('delimiter.array.json', operator)
  add('delimiter.colon.json', operator)
  add('delimiter.comma.json', operator)
  return out
}

export function monacoThemeDefinition(id) {
  const spec = MONACO_THEME_SPECS[id]
  return {
    base: spec.base,
    inherit: true,
    // JSON scopes appended after the theme's own, so a preset that names one
    // explicitly still wins.
    rules: [...jsonRules(spec.rules), ...spec.rules],
    colors: darkChrome(spec.chrome),
  }
}
